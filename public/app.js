...
function openSettings() {
  ...
  var html = '<div class="settings-overlay" onclick="closeSettings(event)"><div class="settings-dialog" onclick="event.stopPropagation()">' +
    ...
    '<div class="settings-section" style="border-top:1px solid var(--border);padding:16px;margin:12px -4px 0 -4px;background:var(--card-bg);border-radius:8px"><div style="font-size:.78rem;font-weight:700;color:var(--muted);margin-bottom:8px">💾 数据备份</div><div class="settings-row"><label>导出所有笔记</label><button class="btn btn-sm" onclick="backupData()" style="margin:0">导出备份</button></div><div class="settings-row"><label>从备份恢复</label><label class="btn btn-sm btn-outline" style="margin:0;cursor:pointer">选择文件<input type="file" id="restore-input" accept=".json" hidden onchange="restoreData(this)"></label></div></div>' +
  '</div></div>';
